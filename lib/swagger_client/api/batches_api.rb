...
module SwaggerClient
  class BatchesApi
    ...
    def create_with_http_info(body, opts = {})
      ...
      fail ArgumentError, "Missing the required parameter 'body' when calling BatchesApi.create" if body.nil?
      # resource path
      local_var_path = "/v1/batches".sub('{format}','json')
    ...
    # @option opts [String] :status filter one or more status. multiple value comma separated
    ...
    def query_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: BatchesApi.query ..."
      end
      # resource path
      local_var_path = "/v1/batches".sub('{format}','json')
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: BatchesApi#query\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
